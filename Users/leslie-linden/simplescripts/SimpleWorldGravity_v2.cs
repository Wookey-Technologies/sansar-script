...
public class SimpleWorldGravity : SceneObjectScript
{
    #region EditorProperties
    [Tooltip("Set default gravity")]
    [DefaultValue("on")]
    [DisplayName("-> Set Earth Gravity")]
    public readonly string SetEarthGravityEvent;

    [Tooltip("Set no gravity")]
    [DefaultValue("off")]
    [DisplayName("-> Set Zero Gravity")]
    public readonly string SetZeroGravityEvent;

    [Tooltip("Set gravity to setting A")]
    [DefaultValue("set_gravity_{INDEX}")]
    [DisplayName("-> Set Gravity By Index")]
    public readonly string SetGravityIndexEvent;

    [Tooltip("Gravity Settings in G's not m/s^2")]
    [DisplayName("Gravity Settings (g)")]
    [Range(0.0f, 5.0f)]
    public readonly List<float> GravityValues;

    [Tooltip("Enable responding to events for this script")]
    [DefaultValue("world_gravity_enable")]
    [DisplayName("-> Enable")]
    public readonly string EnableEvent;

    [Tooltip("Disable responding to events for this script")]
    [DefaultValue("world_gravity_disable")]
    [DisplayName("-> Disable")]
...
If StartEnabled is false then the script will not respond to interactions until an (-> Enable) event is received.")]
    ...
    [Tooltip(@"Update the region tag above by incrementing the version when updating anything in the region.
...
To send or receive events to/from a specific group from outside that group prepend the group name with a > to the event name, e.g. ""my_group>on""")]
    [DefaultValue("")]
    [DisplayName("Group")]
    public string Group = "";
    ...
    protected bool __SimpleDebugging = false;
    protected string __SimpleTag = "";

    protected string GenerateEventName(string eventName)
    {
    ...
    protected void SetupSimple()
    {
        ...
        IDebugger debugger = ScenePrivate.FindReflective<IDebugger>("Simple.Debugger").FirstOrDefault();
    ...
    protected Action SubscribeToAllInternal(string csv, Action<ScriptEventData> callback)
    {
    ...
    protected void SendToAllInternal(string csv, Reflective data)
    {
        if (string.IsNullOrWhiteSpace(csv)) return;
        string[] events = csv.Trim().Split(new char[] { ',' }, StringSplitOptions.RemoveEmptyEntries);
