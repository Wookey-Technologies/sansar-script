...
namespace PewPewExample
{
                ...
                else
                {
                    ...
                    if (targetObject != null)
                    {
                        ...
                        if (target != null)
                        {
                            ...
                            score += target.Hit(holdingAgent, ScenePrivate, ShotHitEvent);
                            shotsHit++;
                            return;
                        }
                    }
    ...
    public class Target : ObjectScript
    {
        ...
        public int Hit(AgentInfo agent, ScenePrivate scene, string _shotHitEvent)
        {
          if (DebugLogging) Log.Write("ShotHitEvent ", ShotHitEvent);
          if (DebugLogging) Log.Write("_shotHitEvent ", _shotHitEvent);

          if(_shotHitEvent != null || _shotHitEvent == ShotHitEvent)
          {
            if (HitSound != null)
            {
            ...
            try
            {
                ...
                Log.Write("Event sent  ", ShotHitEvent);
            }
            catch (System.Exception) { }
            return PointValue;
          }
