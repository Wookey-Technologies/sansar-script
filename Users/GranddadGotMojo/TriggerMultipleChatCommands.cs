...
public class TriggerMultipleChatCommands : SceneObjectScript
{
    #region EditorProperties
    // The commands
    [DefaultValue("Event1")]
    [DisplayName("Chat Commands ->")]
    public readonly string ChatCommands;
    ...
    private List<string> ChatCommandList = new List<string>();

    public override void Init()
    {
        ScenePrivate.Chat.Subscribe(Chat.DefaultChannel, OnChat);
        ParseCommandString(ChatCommands);
    }

    public void ParseCommandString(string CommandsIn)
    {
        ChatCommandList.Clear();
        ChatCommandList = CommandsIn.Split(',').ToList();
    }

    void OnChat(ChatData data)
    {
        foreach (string ChatCommand in ChatCommandList)
        {
            if (data.Message.Contains(ChatCommand))
            {
