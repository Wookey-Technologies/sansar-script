...
namespace ScriptLibrary
{
    ...
    public class ObjectChatCommand : ObjectBase
    {
...
If StartEnabled is false then the script will not respond to interactions until an (-> Enable) event is received.")]
        ...
        protected override void SimpleInit()
        {
            ...
            {
                ScenePublic.Chat.Subscribe(Sansar.Simulation.Chat.DefaultChannel, OnChat);
            }
        ...
        bool IsAccessAllowed(AgentPublic agent)
        {
            bool accessAllowed = true;

            if (CreatorOnly)
            {
                accessAllowed = (agent != null) && (agent.AgentInfo.AvatarUuid == ScenePublic.SceneInfo.AvatarUuid);
            }
