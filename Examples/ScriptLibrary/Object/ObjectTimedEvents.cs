...
namespace ScriptLibrary
{
    ...
    [DisplayName("TimedEvents")]
    public class ObjectTimedEvents : ObjectBase
    {
        ...
        [Tooltip(@"The events to send at the timer interval, alternating with ""On Timer 2nd ->"" if both are specified. Can be a comma separated list of event names. Use >> separator to send alternating events. 'first>>second,third>>fourth' will send 'first' then 'second' and 'third' then 'fourth' then 'first' etc.")]
        [DefaultValue("on")]
...
If StartEnabled is false then the script will not respond to events until an (-> Enable) event is received.")]
        ...
        void NonRepeatingTimer()
        {
            ...
            int eventCount = OnTimer.Split(new string[]{ ">>"}, StringSplitOptions.RemoveEmptyEntries).Length - 1;
