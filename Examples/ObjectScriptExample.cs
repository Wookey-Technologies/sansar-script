...
public class ObjectScriptExample : ObjectScript
{
    ...
    public override void Init()
    {
        ...
        if (RigidBody == null)
        {
            if (!ObjectPrivate.TryGetFirstComponent(out RigidBody))
            {
                ...
                ScenePublic.Chat.MessageAllUsers("There is no RigidBodyComponent attached to this object.");
                return;
            }
        }
    ...
    private void CheckForCollisions(CollisionEventType trackedEvents)
    {
        while (true)
        {
            ...
            {
                ScenePublic.Chat.MessageAllUsers("I hit an avatar!");
            }
            else
            {
                ScenePublic.Chat.MessageAllUsers("I hit an object!");
            }
        }
    }
    ...
    private void UnhandledException(object sender, Exception e)
    {
        ...
        {
            ScenePublic.Chat.MessageAllUsers("Unrecoverable exception happened, the script will now be removed.");
        }
        else
        {
            ScenePublic.Chat.MessageAllUsers("This script will be allowed to continue.");
        }
    }
}